...
const ReminderModal = ({ event, isOpen, onClose, onReminderSet }) => {
  ...
  const { joinUserRoom, fetchNotifications } = useNotifications()
    ...
    try {
      if (isAuthenticated) {
        ...
        const response = await axios.post(
          'http://localhost:5001/api/users/reminders',
          {
            eventId: event.id,
  ...
  const handleRemove = async (eventId) => {
    ...
    try {
      if (isAuthenticated) {
        ...
        if (success) {
          // Refetch notifications after successful removal
          fetchNotifications();
          onReminderSet(false)
          onClose()
        } else {
  ...
  const handleTestEmail = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      await axios.post(
        'http://localhost:5001/api/test-email',
        {},
        {
          headers: {
            'x-auth-token': token,
