...
app.delete("/api/users/notifications/:notificationId", auth, async (req, res) => {
  try {
    console.log(`[Backend] DELETE /api/users/notifications/:notificationId called with ID: ${req.params.notificationId}`);
    ...
    if (notificationIndex === -1) {
      console.log(`[Backend] Notification with ID ${req.params.notificationId} not found for this user.`);
      return res.status(404).json({ msg: "Notification not found for this user." });
    }

    // Remove the notification from the user's notifications array using splice
    user.notifications.splice(notificationIndex, 1);

    await user.save();
    console.log(`[Backend] Notification with ID ${req.params.notificationId} removed successfully.`);
    res.json({ msg: "Notification removed" });
  } catch (err) {
    console.error('[Backend] Error deleting notification:', err.message);
    res.status(500).send("Server error");
  }
});
