...
export const removeReminder = async (eventId) => {
  try {
    ...
    if (isAuthenticated()) {
      ...
      // Also remove the associated notification
      try {
        console.log(`Attempting to find notification for eventId: ${eventId}`);
        const notifications = await api.get('/users/notifications');
        console.log('Fetched notifications:', notifications.data);
        const notificationToRemove = notifications.data.find(n => n.eventId === eventId);

        if (notificationToRemove) {
          console.log('Found notification to remove:', notificationToRemove);
          await api.delete(`/users/notifications/${notificationToRemove._id}`);
          console.log(`Successfully sent delete request for notification ID: ${notificationToRemove._id}`);
        } else {
          console.log(`No notification found for eventId: ${eventId}`);
        }
      } catch (error) {
